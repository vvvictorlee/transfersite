...
#### 请求
##### 参数说明：
- method  方法名称 get_pairtokens_info
- address  指定账户地址 可选
##### 请求示例
```
curl  -X POST --url http://192.168.38.227:3535/farm/   -H "Content-Type: application/json"  -d '{
...
- symbol        交易对铺子币代币符号
- released      交易对铺子币已发行量
- ydays_swps    昨天交易对铺子币分配SWP代币金额 
- yourbalance   指定账户交易对铺子币余额 
- yourshare     指定账户交易对铺子币份额 
- verified      交易对铺子币是否认证
#####
```
[
...
#### 请求
##### 参数说明：
- method  方法名称 get_eth_pairs_info
- address  指定账户地址 可选
##### 请求示例
```
curl  -X POST --url http://192.168.38.227:3535/farm/   -H "Content-Type: application/json"  -d '{
